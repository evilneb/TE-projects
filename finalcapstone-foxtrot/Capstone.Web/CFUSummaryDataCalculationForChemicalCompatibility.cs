...
{
    public static class CFUSummaryDataCalculationForChemicalCompatibility
    {
        public static int FindLengthOfSublistAmountsPerSublist(List<ChemicalCompatibility> bigListOfSummaryData)
        {
            string currentRep = "0";
            int sublistLength = 0;
            foreach (ChemicalCompatibility row in bigListOfSummaryData)
            {
        ...
        public static int FindRepitionAmountPerSublist(List<ChemicalCompatibility> bigListOfSummaryData)
        {
            string currentRep = "0";
            int repCount = 0;
            foreach (ChemicalCompatibility row in bigListOfSummaryData)
        ...
        public static List<List<ChemicalCompatibility>> ChunkListGeneration(List<ChemicalCompatibility> summaryChemicalCompatibiltyData)
        {
            int subListLength = FindLengthOfSublistAmountsPerSublist(summaryChemicalCompatibiltyData);
            List<List<ChemicalCompatibility>> chunkList = new List<List<ChemicalCompatibility>>();

            for (int i = 0; i < summaryChemicalCompatibiltyData.Count; i += subListLength)
            {
                List<ChemicalCompatibility> currentList = new List<ChemicalCompatibility>();
                for (int j = i; j < subListLength + i; j++)
                {
                    currentList.Add(summaryChemicalCompatibiltyData[j]);
                }
                currentList.Add(new ChemicalCompatibility());
                chunkList.Add(currentList);
            }
            return chunkList;
        }

        public static List<List<ChemicalCompatibility>> CalcualteAndAddAverageCFUValueToEachSubListAndReturnThatChunkList(List<List<ChemicalCompatibility>> chunkList)
        {
            int lastRowIndexValue = FindLengthOfSublistAmountsPerSublist(chunkList[0]);
            ...
            foreach (List<ChemicalCompatibility> subList in chunkList)
            {
